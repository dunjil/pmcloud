...
pcm_gen=pd.DataFrame()
pcn_b_gen=pd.DataFrame()
pcn_c_gen=pd.DataFrame()
ptm_d_gen=pd.DataFrame()
ptm_e_gen=pd.DataFrame()
pmf_gen=pd.DataFrame()
pmi_gen=pd.DataFrame()
pms_gen=pd.DataFrame()
pmn_gen=pd.DataFrame()
pmw_gen=pd.DataFrame()
...
def backlog(request):
    ...
    Q(job_status__in=['A', 'AE', 'C', 'IP', 'JE', 'JR', 'JV', 'JW', 'C']) &
    Q(jobtas__task_status__in=['A', 'ER', 'IP', 'JW', 'NE']) & 
    Q(jobtas__ja_orgn_id__orgn_code__in=['ELECSMART'
    'ELECTSHOP', 'FABWKSHOP', 'HDWKSHOP','INST_SUPP','INSTWKSHOP', 'MACHINSHOP', 'MC_WKSHOP', 'MECHWKSHOP', 'OVHDCRANE']) &
    ~Q(next_step__in=["RM","JV"]) &
    Q(target_end_date__gte=(datetime.datetime(2010, 1, 1, 0, 0, 1))) &
    ~Q(rqtr__in=['PACER']) &
